...
export default class TurnAlert {
  ...
  static async create(data) {
    ...
    if (combat.canUserModify(<User>getGame().user, 'update')) {
      ...
      log(
        `Turn Alert | User ${getGame().userId} does not have permission to edit combat ${
          alertData.combatId
        }; sending createAlert request...`,
      );
  ...
  static async update(data: TurnAlert) {
    ...
    if (combat.canUserModify(<User>getGame().user, 'update')) {
      ...
      log(
        ` User ${getGame().userId} does not have permission to edit combat ${
          data.combatId
        }; sending updateAlert request...`,
      );
